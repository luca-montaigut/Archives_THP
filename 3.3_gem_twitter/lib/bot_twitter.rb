...
abonnés = ['@simon_doutrelow','@BobHope96928725','@LeporattiR']

def like(client, nb)
  client.favorite(client.search("#bonjour_monde", result_type: "recent").take(nb))
end


def follow(client)
  tab = []
  client.search("#bonjour_monde", result_type: "recent").take(200).each {|tweet| tab << (tweet.user)}
  client.follow(tab.uniq)
  #possibilité d'améliorer pour affichier les follows réalisés 
end
